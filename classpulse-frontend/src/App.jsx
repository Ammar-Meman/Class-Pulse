...
function App() {
  ...
  return (
    <div className={`min-h-screen flex flex-col items-center justify-center p-6`}>
      ...
      <nav className={`fixed top-4 sm:top-8 left-1/2 -translate-x-1/2 z-50 glass-panel px-4 sm:px-8 py-3 sm:py-4 rounded-[1.5rem] sm:rounded-[2rem] flex gap-4 sm:gap-8 items-center border-2 border-current border-opacity-20 shadow-brutal w-[95%] sm:w-auto justify-center`}>
        <div className="flex gap-4 sm:gap-6 items-center pr-3 sm:pr-4 border-r-2 border-current border-opacity-10">
          <Link to="/" className="text-[10px] sm:text-sm font-black uppercase tracking-tighter hover:scale-110 transition-transform flex items-center gap-2">
            Student
          </Link>
          <Link to="/teacher-login" className="text-[10px] sm:text-sm font-black uppercase tracking-tighter hover:scale-110 transition-transform flex items-center gap-2">
            Teacher
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <button 
            onClick={toggleTheme}
            className={`relative w-16 sm:w-20 h-7 sm:h-9 rounded-full border-2 border-current transition-all overflow-hidden group shadow-sm active:scale-95
              ${isNature ? "bg-brutal-green text-black" : "bg-brutal-red text-white"}`}
          >
            <div className={`absolute inset-0 flex items-center justify-center font-mono font-black text-[6px] sm:text-[8px] uppercase tracking-tighter transition-all duration-300
              ${isNature ? "translate-x-0" : "translate-x-full opacity-0"}`}>
              Nature
            </div>
            <div className={`absolute inset-0 flex items-center justify-center font-mono font-black text-[6px] sm:text-[8px] uppercase tracking-tighter transition-all duration-500
          ...
          <button 
            ...
            className={`flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border-2 border-current font-mono font-black text-[7px] sm:text-[9px] uppercase tracking-tighter transition-all active:scale-90
              ${isNature 
          ...
          >
            ...
            <span className="hidden xs:inline">Wall #</span>{wallpaperIndex + 1}
          </button>
        </div>
      </nav>
